...
namespace GiaoDien.Forms
{
    public partial class fUpdateLS : Form
    {
        ...
        public fUpdateLS(DataGridView selectedRow, fManager fManager)
        {
            ...
            cbTinhTrang.Text = selectedRow.SelectedRows[0].Cells["Tình trạng"].Value?.ToString() ?? "Tốt";
            tbGhiChu.Text = selectedRow.SelectedRows[0].Cells["Ghi chú"].Value?.ToString() ?? string.Empty;
        ...
        private void btnConfirmUpdateLS_Click(object sender, EventArgs e)
        {
            ...
            // Thực hiện cập nhật
            string query = @"UPDATE dbo.VatNuoi SET IDVatNuoi = @id , loai = @loai , tenGiong = @tenGiong , tenChuong = @chuong , gioitinh = @gioiTinh , ngaySinh = @ngaySinh , canNang = @canNang , IDNguoiDung = @IDNguoiDung , tinhTrangSucKhoe = @tinhTrang , ghiChu = @ghiChu WHERE IDVatNuoi = @idCu ";

            int rows = DataProvider.Instance.ExecuteNonQuery(query, new object[]
            {
                idMoi, loai, tenGiong, tenChuong, gioiTinh, ngaySinh, canNang, idNguoiDung, tinhTrang, ghiChu, idCu
            });
